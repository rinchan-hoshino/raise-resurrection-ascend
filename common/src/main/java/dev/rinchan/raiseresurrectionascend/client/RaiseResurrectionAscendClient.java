...
public final class RaiseResurrectionAscendClient {
    ...
    private static final KeyMapping.Category GIVE_UP_CATEGORY = KeyMapping.Category.register(
        Identifier.fromNamespaceAndPath(RaiseResurrectionAscend.MOD_ID, "general")
    );
    ...
    public static KeyMapping.Category giveUpCategory() {
        return GIVE_UP_CATEGORY;
    }
    ...
    public static void renderHud(GuiGraphicsExtractor graphics) {
        ...
        for (int index = 0; index < causeLineCount; index++) {
            graphics.centeredText(minecraft.font, causeLines.get(index), centerX, centerY + index * 12, 0xFF8080);
        }
        graphics.centeredText(minecraft.font, recovery, centerX, recoveryY, 0xFFFFFF);
        graphics.centeredText(minecraft.font, giveUp, centerX, giveUpY, 0xFFB0B0);
        if (localHeldTicks > 0) {
